...
class CursetTests(unittest.TestCase):
    verbose = 1
    ...
    def test_floating_point(self):
        ...
        for i in range(loops):
            n = random()
            s = Curset(construct(n,precision=precision))
            fs = float(s)
            diff = abs(n - fs)
...
'''.format(n,fs,diff,log(diff,1/2),depth))
            ...
            for j in sorted_s:
                if i+j in s:
                    try:
                        ...
                        if str(e) == "'curset number is not in the universe'":
                            print('  skip: curset outside universe')
                        else:
                            raise
    ...
    def test_multiplication(self):
        ...
        print('\ndone.\n\nmultiplication success: these curset numbers operate according to their given labels.')
    ...
    def Xtest_inversion(self):
        ...
        for i in sorted(s):
            ...
            try:
                ...
                print(sys.exc_info())
        print('\ndone.\n\ninversion success: these curset numbers operate according to their given labels.')
